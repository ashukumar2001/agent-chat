...
import { MessageType, type OutgoingMessage } from "@/types/ai-types";
 ...
 */
type UseAgentChatOptions<State> = Parameters<typeof useChat>[0] & {
...
export function useAgentChat<State = unknown>(
    ...
    function doGetInitialMessages(
        ...
        const promise = getInitialMessagesFetch(getInitialMessagesOptions) as Promise<UIMessage[]>;
    ...
    useEffect(() => {
        ...
        // when resolved.
        requestCache.set(agentUrlString, initialMessagesPromise! as Promise<UIMessage[]>);
        return () => {
    ...
    async function aiFetch(
        ...
        agent.addEventListener(
            ...
            (event) => {
                ...
                if (data.type === MessageType.CF_AGENT_USE_CHAT_RESPONSE) {
                    if (data.id === id) {
                        if (data.body) {
                            const encodedChunk = new TextEncoder().encode(data.body);
        ...
        agent.send(JSON.stringify(requestMessage));
    ...
    useEffect(() => {
        ...
        function onMessages(event: MessageEvent) {
            ...
            if (data.type === MessageType.CF_AGENT_CHAT_MESSAGES) {
                useChatHelpers.setMessages(data.messages as UIMessage[]);
            }
        }
